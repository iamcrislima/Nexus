...
interface ProductItem {
  ...
  desc: string;
  kpi: string;
  status: "ativo" | "atencao" | "setup" | "soon" | "disabled";
}
...
const MOCK_BIG_NUMBERS: Record<Period, BigNumber[]> = {
  "7d": [
...
const MOCK_PRODUCTS: ProductGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "msg",       initials: "ME", color: "#EC4899", name: "Mensageria", desc: "Comunicação · em breve", kpi: "", status: "soon" },
    ],
  },
];
...
const MOCK_EFFICIENCY: Record<Period, EfficiencyItem[]> = {
  "7d":  [
...
const MOCK_SPARKLINE: Record<Period, number[]> = {
  "7d":  [30, 27, 35, 28, 45, 40, 68],
  "30d": [18, 22, 15, 28, 32, 19, 24, 30, 27, 35, 28, 22, 38, 42, 35, 28, 45, 40, 38, 50, 44, 52, 48, 55, 50, 62, 58, 65, 70, 68],
  "90d": [12, 18, 14, 20, 22, 17, 25, 28, 24, 30, 18, 22, 15, 28, 32, 19, 24, 30, 27, 35, 28, 22, 38, 42, 35, 28, 45, 40, 38, 50, 44, 52, 48, 55, 50, 62, 58, 65, 70, 68, 72, 78, 74, 80, 76, 82, 85, 88, 84, 90],
};

const ALERT_ICON: Record<AlertItem["type"], React.ReactNode> = {
  error:   <FAIcon icon="fa-solid fa-triangle-exclamation" style={{ color: "#ef4444" }} />,
  warning: <FAIcon icon="fa-solid fa-circle-exclamation"  style={{ color: "#f59e0b" }} />,
  info:    <FAIcon icon="fa-solid fa-circle-info"          style={{ color: "#6366f1" }} />,
  success: <FAIcon icon="fa-solid fa-circle-check"         style={{ color: "#10b981" }} />,
};
      ...
      <div className="nexus-home__body">
          ...
          {bigNumbers.map((n) => (
            <div key={n.label} className="nh-bignum">
              ...
              <div className={`nh-bignum__delta nh-bignum__delta--${n.positive ? "positive" : "negative"}`}>
                <FAIcon icon="fa-solid fa-circle-dot" style={{ fontSize: 7 }} />
                {n.delta}
              </div>
            </div>
          ))}
        ...
        <div className="nexus-home__main-grid">
              ...
              {MOCK_PRODUCTS.map((group) => (
                <div key={group.category}>
                  ...
                  {group.items.map((p) => {
                    ...
                    return (
                      <div key={p.id} className={`nh-product-card${muted ? " nh-product-card--muted" : ""}`}>
                        <div
          ...
          <div className="nexus-home__side">
            <div className="nh-card">
              ...
              {MOCK_FEED.map((item) => (
                <div key={item.text} className="nh-feed-item">
        ...
        <div className="nexus-home__bottom-grid">
          <div className="nh-card">
            ...
            <div className="nh-card__title" style={{ marginTop: 8 }}>Documentos Criados ({period.toUpperCase()})</div>
          ...
          <div className="nh-card">
            ...
            {MOCK_ALERTS.map((alert) => (
              <div key={alert.text} className={`nh-alert nh-alert--${alert.type}`}>
